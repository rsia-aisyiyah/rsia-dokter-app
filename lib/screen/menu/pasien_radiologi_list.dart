...
class PasienRadiologiList extends StatefulWidget {
  final bool ralan;
  final bool ranap;

  const PasienRadiologiList({
    super.key,
    this.ralan = false,
    this.ranap = false,
  });
  ...
  @override
  State<PasienRadiologiList> createState() => PasienListSRadiologitate();
}

class PasienListSRadiologitate extends State<PasienRadiologiList> {
  ...
  Future fetchPasien() async {
    ...
    var res = await Api().postData(filterData, strUrl);
  ...
  Future _fetchSearch(data) async {
    var res = await Api().postData(data, '/pasien/radiologi');
    if (res.statusCode == 200) {
      var body = json.decode(res.body);
      return body;
    } else {
  ...
  void doFilter() async {
    setState(() {
      isLoding = true;
      isFilter = true;
      filterData['status'] = widget.ralan ? 'Ralan' : 'Ranap';
    });
